...
def state_filter(data,shp,st_names,bc,inst_type,moi,gen, caste,clas,sector):
    t_df = data[(data["basic_course"] == bc)& data.instruction_med.isin(moi) &data["gender"].isin(gen) & data["social_group"].isin(caste) & data["clas"].isin(clas) & data["place"].isin(sector)]
    ...
    return st, st_c
def district_filter(data,shp,st_c,d_names,bc,inst_type,moi,gen, caste,clas,sector):
    t_df = data[(data["state_code"] == st_c)&(data["basic_course"] == bc) & data.instruction_med.isin(moi)& data["gender"].isin(gen) & data["social_group"].isin(caste) & data["clas"].isin(clas) & data["place"].isin(sector)]
...
#global sc
sc = 36
Dist, dist_c = district_filter(df,d_shp,sc,district_n,"School","government",["others"],gen,caste,clas,sector)
District_geo = GeoJSONDataSource(geojson = Dist.to_json())
dist_source = ColumnDataSource(dist_c)

state,state_c = state_filter(df,s_shp,state_n,"School","government",["others"],gen,caste,clas,sector)
...
lan = Div(text = """<u>Choose medium of instruction below <u>""")
lang = CheckboxGroup(labels=["English","Others"], active=[1])
...
def update_data(attrname, old, new):
    ...
    for i in cls.active:
        claas.append(clas[i])
    s_d,state_c = state_filter(df,s_shp,state_n,bs,ins,medi,gr,sg,claas,sctor)
    d_d, dist_c = district_filter(df,d_shp,sc,district_n,bs,ins,medi,gr,sg,claas,sctor)   
...
def my_tap_handler(attr,old,new):
    if len(new)!= 0:
        ...
        print(sc,bs,ins,medi,gr,sg,claas,sctor)
        d_d, dist_c = district_filter(df,d_shp,sc,district_n,bs,ins,medi,gr,sg,claas,sctor)
        print(d_d.head())
        District_geo.geojson = d_d.to_json()
        dist_source.data = dist_c
...
lang.on_change("active",update_data)
...
widgets = column(ty,type_l,lan,lang,El,scl_lvl,pl,sctr,gender,gend,sog,cst,inc,cls)
